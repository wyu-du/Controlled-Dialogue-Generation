...
def decode_time(args):
    ...
    if args.timestamp == '0':
        ...
        tokenizer = GPT2Tokenizer.from_pretrained(f"{args.model_name}_convai2_raw_ft_2021-05-06-10-20-44/checkpoint-5000")
    print(f"Vocab size: {len(tokenizer)}")
    
    if args.timestamp == '0':
        model = Seq2SeqModel.from_pretrained(f"{args.model_name}")
    else:
        model = Seq2SeqModel.from_pretrained(f"{args.model_name}_convai2_raw_ft_2021-05-06-10-20-44/checkpoint-5000")
    model = model.to('cuda:0')
    print('Model loaded!!!')
...
if __name__ == '__main__':
    ...
    p.add_argument('-c', '--ckpt', type=str, default="8946", 
                   help="Model checkpoint")
    p.add_argument('-time', '--timestamp', type=str, default='2021-04-14-09-25-54', 
                   help="Model checkpoint")
